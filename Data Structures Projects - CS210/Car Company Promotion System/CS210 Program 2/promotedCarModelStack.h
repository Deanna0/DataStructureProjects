...
private:  
  ...
  PromotedModel maxModel;
  PromotedModel minModel;
  PromotedModel car;
  PromotedModel topOfCarStack;
  PromotedModel poppedItem;

  std::vector<PromotedModel> carNameAndPrice = std::vector<PromotedModel>(); //Our vector "stack" of Promoted Model objects
  std::vector<PromotedModel> highestAndLowestPrice = std::vector<PromotedModel>(); //Vector to keep track of min & max prices & has the form of max price then min price (min price will always be on top)
